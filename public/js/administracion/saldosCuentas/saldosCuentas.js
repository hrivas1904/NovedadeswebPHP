...
function renderFilaPesos(r) {
    ...
    return (
        ...
        r.nombre +
        '">' +
        '<td class="fw-semibold" style="color:#1B2A4A;">' +
        r.nombre +
        asterisco +
        "</td>" +
        '<td><input readonly type="text" class="form-control form-control-sm text-end input-saldo-inicial" ' +
        estiloInput +
        ' value="' +
        valor +
        '" data-original="' +
        ...
        fmtPesos(r.movimientos_ejecutados) +
        "</td>" +
        '<td class="text-end fw-bold" style="color:#28a745;">' +
        fmtPesos(r.saldo_actual) +
        "</td>" +
        "</tr>"
    );
}
...
function renderFilaUsd(r) {
    ...
    return (
        ...
        r.nombre +
        " *</td>" +
        '<td><input readonly type="text" class="form-control form-control-sm text-end input-saldo-inicial" style="background:#FFF9E0;" value="' +
        valor +
        '" data-original="' +
        valor +
        '"></td>' +
        '<td class="text-end" style="color:#28a745;">' +
        fmtUsd(r.movimientos_ejecutados) +
        "</td>" +
        '<td class="text-end fw-bold" style="color:#28a745;">' +
        fmtUsd(r.saldo_actual) +
        "</td>" +
        "</tr>"
    );
}
...
function cargarSaldosCuenta(mes) {
    $.get(SALDOS_CUENTA_ROUTES.data, { mes: mes }, function (data) {
        $("#bodySaldosCuentaPesos").html(
            data.pesos.map(renderFilaPesos).join(""),
        );
        $("#bodySaldosCuentaUsd").html(data.usd.map(renderFilaUsd).join(""));

        const totPesosInicial = data.pesos.reduce(
            (a, r) => a + Number(r.saldo_inicial),
            0,
        );
        const totPesosMov = data.pesos.reduce(
            (a, r) => a + Number(r.movimientos_ejecutados),
            0,
        );
        const totPesosActual = data.pesos.reduce(
            (a, r) => a + Number(r.saldo_actual),
            0,
        );
        ...
        const totUsdInicial = data.usd.reduce(
            (a, r) => a + Number(r.saldo_inicial),
            0,
        );
        const totUsdMov = data.usd.reduce(
            (a, r) => a + Number(r.movimientos_ejecutados),
            0,
        );
        const totUsdActual = data.usd.reduce(
            (a, r) => a + Number(r.saldo_actual),
            0,
        );
        $("#totalUsdInicial").text(fmtUsd(totUsdInicial));
        $("#totalUsdMov").text(fmtUsd(totUsdMov));
...
$(document).ready(function () {
    ...
    $(document).on("blur", ".input-saldo-inicial", function () {
        ...
        if (raw === "") {
            $.post(
                SALDOS_CUENTA_ROUTES.eliminar,
                { cuenta: cuenta, mes: mes },
                function () {
        ...
        $.post(
            SALDOS_CUENTA_ROUTES.guardar,
            { cuenta: cuenta, mes: mes, monto: monto },
            function () {
                cargarSaldosCuenta(mes);
            },
        );
