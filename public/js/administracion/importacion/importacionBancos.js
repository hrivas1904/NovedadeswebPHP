...
const COLUMNAS_POR_COMBINACION = {
    'MACRO|PEGADO':               ['Fecha', 'Nro', 'CódOp', 'Descripción', 'Importe', 'Saldo'],
    'MACRO|CONCILIACION':         ['Fecha', 'Operación', 'Concepto', 'Detalle', 'Nro', 'Importe', '—', 'Saldo'],
    'NACION|PEGADO':              ['Fecha+Hora', 'Descripción', 'Crédito', 'Débito'],
    'NACION|CONCILIACION':        ['Fecha', '—', 'Concepto', 'Detalle', 'Importe (con signo)', 'Saldo'],
    'FRANCES (986)|PEGADO':       ['Fecha', 'Descripción', 'CódOp', 'Crédito', 'Débito', 'Saldo'],
    'FRANCES (986)|CONCILIACION': ['Fecha', '—', 'Concepto', 'Detalle', 'Importe (con signo)', 'Saldo'],
    'FRANCES (1001)|PEGADO':      ['Fecha', 'Descripción', 'CódOp', 'Crédito', 'Débito', 'Saldo'],
    'FRANCES (1001)|CONCILIACION':['Fecha', '—', 'Concepto', 'Detalle', 'Importe (con signo)', 'Saldo'],
};
...
function actualizarHeaderColumnas() {
    ...
    $('#headerColumnasBancos').html(
        columnas.length
            ? '<strong>Columnas esperadas:</strong>&nbsp; ' + columnas.join(' &nbsp;|&nbsp; ')
            : ''
    );
}

function actualizarPlaceholder() {
    $('#contenidoBancos').attr('placeholder', PLACEHOLDER_POR_BANCO[bancoSeleccionado] || 'Pegá los movimientos...');
}
...
function renderPreviewBancos() {
    ...
    let html = '<div id="previewBancosWrapper" class="mt-3">' +
        '<div class="d-flex justify-content-between align-items-center mb-2">' +
        '<span class="text-muted small">' + filasBancos.length + ' movimientos</span>' +
        '<button type="button" id="btnConfirmarBancos" class="btn btn-primary btn-sm">Importar ' + filasBancos.length + ' movimientos</button>' +
        '</div>' +
    ...
    filasBancos.forEach(function (r, i) {
        ...
        html += '<tr>' +
            '<td>' + r.fecha + '</td>' +
            '<td>' + r.banco + '</td>' +
            '<td><select class="form-select form-select-sm select-concepto-banco" data-idx="' + i + '">' + optsFila + '</select></td>' +
            '<td><input type="text" class="form-control form-control-sm input-subconcepto-banco" data-idx="' + i + '" value="' + (r.subconcepto || '') + '"></td>' +
            '<td>' + r.detalle + '</td>' +
...
function procesarBancos() {
    ...
    $.post(IMPORTACION_ROUTES.bancosPreview, {
        contenido: contenido,
        banco: bancoSeleccionado,
        formato: formatoSeleccionado,
    }, function (data) {
        filasBancos = data.rows;
        $('#msgBancos').text(data.mensaje).css('color', filasBancos.length ? 'green' : 'inherit');
        renderPreviewBancos();
    });
}

function procesarArchivoBancos(archivo) {
    const formData = new FormData();
    formData.append('archivo', archivo);
    formData.append('banco', bancoSeleccionado);
    formData.append('formato', formatoSeleccionado);

    $('#msgBancos').text('Leyendo archivo...').css('color', 'inherit');

    $.ajax({
        url: IMPORTACION_ROUTES.bancosPreview,
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        ...
        error: function () {
            $('#msgBancos').text('Error al leer el archivo.').css('color', 'red');
        }
    });
}
...
$(document).ready(function () {
    if (!$('#msgBancos').length) {
        $('#contenidoBancos').after('<label class="mt-2" id="msgBancos"></label>');
    }
    ...
    $(document).on('change', '#inputArchivoBancos', function () {
        if (this.files && this.files[0]) {
            $('#contenidoBancos').val(''); // el archivo tiene prioridad sobre el texto pegado
            procesarArchivoBancos(this.files[0]);
        }
    });
    ...
    $(document).on('click', '#btnConfirmarBancos', function () {
        ...
        $.post(IMPORTACION_ROUTES.bancosConfirmar, { rows: filasBancos }, function (data) {
            $('#msgBancos').text('✓ ' + data.insertados + ' movimientos importados.').css('color', 'green');
            $('#contenidoBancos').val('');
            filasBancos = [];
            renderPreviewBancos();
        }).fail(function (xhr) {
