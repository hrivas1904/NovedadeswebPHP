...
function renderPreviewCaja() {
    ...
    let html =
        '<div id="previewCajaWrapper" class="mt-3">' +
        '<div class="d-flex justify-content-between align-items-center mb-2">' +
        '<span class="text-muted small">' +
        filasCaja.length +
        " movimientos</span>" +
        '<button type="button" id="btnConfirmarCaja" class="btn btn-primary btn-sm">Importar ' +
        filasCaja.length +
        " movimientos</button>" +
        "</div>" +
    ...
    filasCaja.forEach(function (r, i) {
        ...
        html +=
            ...
            r.fecha +
            "</td>" +
            '<td><select class="form-select form-select-sm select-concepto-caja" data-idx="' +
            i +
            '">' +
            optsFila +
            "</select></td>" +
            '<td><input type="text" class="form-control form-control-sm input-subconcepto-caja" data-idx="' +
            i +
            '" value="' +
            (r.subconcepto || "") +
            '"></td>' +
...
function procesarCaja() {
    ...
    $.post(
        IMPORTACION_ROUTES.cajaPreview,
        { contenido: contenido },
        function (data) {
            filasCaja = data.rows;
            $("#msgCaja")
                .text(data.mensaje)
                .css("color", filasCaja.length ? "green" : "inherit");
            renderPreviewCaja();
        },
    );
}
...
$(document).on("click", "#btnConfirmarCaja", function () {
    ...
    $.post(
        IMPORTACION_ROUTES.cajaConfirmar,
        { rows: filasCaja },
        function (data) {
            $("#msgCaja")
