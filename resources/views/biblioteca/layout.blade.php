...
@php($canManage=$canManage && !$preview)
    ...
    <nav class="bib-nav" aria-label="Secciones de la biblioteca">
        ...
        @if($canManage)<a href="{{ route('biblioteca.coverage') }}" class="{{ request()->routeIs('biblioteca.coverage','biblioteca.acceptance.register')?'active':'' }}">Categorías y firmas</a>@endif
