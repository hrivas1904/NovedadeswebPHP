...
<form action="{{ route('biblioteca.search') }}" class="bib-search"><label for="bib-q">Buscar en toda la biblioteca</label><div class="input-group"><input class="form-control" name="q" id="bib-q" placeholder="Puesto, tema, código o palabra clave"><button class="btn btn-primary" type="submit"><i aria-hidden="true" class="fa-solid fa-magnifying-glass"></i><span class="ms-2">Buscar</span></button></div><small>{{ count($entries) }} documentos disponibles · Cuatro colecciones</small></form></section>
<div class="bib-collections">
