...
class AnalisisController extends Controller
{
    ...
    public function comparativaDiarioView()
    {
        $pdo  = DB::connection()->getPdo();
        $stmt = $pdo->prepare('CALL SP_FF_ANALISIS_COMPARATIVO_PRESUPUESTO_DIARIO()');
        $stmt->execute();
        ...
        $periodoLabels = $periodos->map(function ($p) {
            return mb_strtoupper(Carbon::parse($p)->locale('es')->isoFormat('ddd DD/MM'));
        });
        ...
        return view('administracion.analisis.comparativaDiario', [
            'periodos'      => $periodos,
            'periodoLabels' => $periodoLabels,
            'resumen'       => $periodos->map(fn($p) => $resumenPorPeriodo->get($p))->values(),
        ])->with('detalle', $detalle);
